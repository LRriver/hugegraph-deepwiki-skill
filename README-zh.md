...
这个仓库提供两个可安装的 Apache HugeGraph 项目族仓库知识助手：
...
重点是让 Claude Code 和 Codex 能快速回答这两个源码仓库相关的问题，包括架构、模块、API、配置、工作流、示例和实现细节。

DeepWiki 是底层知识查询和 MCP 传输通道。两个 skill 都会调用官方 DeepWiki MCP endpoint：
...
日常问答不会 clone HugeGraph 仓库到本地检索，而是使用线上 DeepWiki + 本地缓存的方式：
...
HugeGraph 数据库相关问题使用 `hugegraph-deepwiki-skill`，HugeGraph AI/RAG 相关问题使用 `hugegraph-ai-deepwiki-skill`。两个助手保持独立，避免回答时混用源码仓库上下文。
